            ...
            if self.creds and self.creds.expired and self.creds.refresh_token:
                ...
                if not os.path.exists(self.credentials_file):
                    raise RuntimeError(
                        ...
                        "Mount credentials.json into the container and retry."
                    )
                if os.path.exists('/.dockerenv') and not os.path.exists(self.token_file):
                    raise RuntimeError(
                        f"Google token file not found at '{self.token_file}'. "
                        "Generate token.json locally and place it in backend/google-auth before using sync in Docker."
                    )
